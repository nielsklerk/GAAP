...
def create_psf_from_psf_grid(psf_grid, size, num_to_combine):
    ...
    central = psfs[Ng//2 - num_to_combine//2:Ng//2 + num_to_combine //
                   2, Ng//2 - num_to_combine//2:Ng//2 + num_to_combine//2]
...
def main():
    def download_file(catalog_file):
        ...
        subprocess.run(cmd)
    max_workers = 4
    data_folder = '/net/vdesk/data2/deklerk/GAAP_data/catalog_files'
    ...
    for tile_index in tile_indeces:
        ...
        download_file(tile_files['FINAL-CAT'][0])

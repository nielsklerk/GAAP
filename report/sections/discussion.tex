...
This work developed the \gls{x-bap} method, which computes the aperture flux in different bands by compensating for the effect that the \gls{psf} has on the observation. The simulations show that the pre-seeing aperture flux is recovered and that the error on the measurement can accurately be estimated. Applying \gls{x-bap} to Euclid observations shows that consistent colors are recovered, which can be used for different applications, such as categorization and determining photometric redshift.
...
\gls{x-bap} should improve the accuracy of the color measurements of sources. This should allow for more accurate \gls{sed} fitting of sources. This gives more accurate photo-z, which gives more accurate cosmology

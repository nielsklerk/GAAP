...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{results/figures/simulation/deconvolution_stability.pdf}
...
\begin{figure}[H]
    ...
    \caption{The measured flux normalized by the true flux for different weights and PSF sizes. Flux measurements with weights smaller than the PSF are less accurate. The same holds for weights with a standard deviation $\lesssim 9\%$ of the image size. The latter is the same result found in \autoref{fig: deconvolution stability}. The yellow line denotes the border of 1$\%$ error}
...
\begin{figure}[H]
    ...
    \caption{Color-color diagram of the stars in the field 53.00 -28.00. The colors are derived from the flux in the Rubin filters g, r, and i. The sources in the image are those with a point-source probability> 90\%.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{results/figures/analysis/3d_plot.png}
    \caption{Caption}
    \label{fig: 3d}
\end{figure}

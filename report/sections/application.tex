\chapter{Survey Application}\label{ch: survey application}
After testing and validating \gls{x-bap} in the simulations, \gls{x-bap} is applied to the \gls{mer} mosaics of \gls{q1}. The colors derived from the flux measurements are compared to colors derived from the 1\gls{fwhm} flux measurements in the \gls{mer} catalog. Thereafter, the colors are used to classify sources in the \gls{mer} mosaics, and the result is compared to the \gls{phz} catalog classifications. 
...
Each \gls{mer} mosaic has a corresponding \gls{psf} grid per band. This can be used to find a more accurate \gls{psf} for each region of the image by interpolating between grid points. However, in \gls{x-bap} this is computationally expensive as the Fourier transform of the \gls{psf} in \autoref{eq: numerical deconvolution} then needs to be recalculated for every single source. Instead of that approach, this work averages the center 100 \glspl{psf} in the grid to obtain a single \gls{psf} that represents the seeing across the entire image, which is used for the aperture flux. This approach reduces the precision of the flux measurement, but substantially reduces the computational cost.
...
%     \label{fig: Chimney}
% \end{figure}
% The sources that form a vertical line are the saturated stars, which are point sources.  This means that after convolution with the PSF, they have the same shape as the PSF. These sources can be stacked to create an estimate for the PSF in that image. 


\subsection{Noise Square}

Besides the \gls{rms} maps, information about the correlation between pixels is needed to estimate the error on the aperture flux measurement. For this a noise square is used, which is a cutout of the background in the observation. To find this square, cutouts are taken of the observation in a grid-like pattern. The cutouts are 128x128 pixels. This square should contain no sources, so it should have rather uniform pixel values. For each of the cutouts, the variance is computed and the cutout with the smallest variance is used as noise square. However, for some of the \gls{mer} tiles, the observations do not cover the full tile, resulting in large regions with zeroes. So a quality cut is made to noise square, demanding that the square has at least 90\% non-zero pixel values. This noise square is used to calculate the correlation between the pixels for the MER mosaics (following \Cref{subsec: Noise from RMS}).
...
To separate sources into groups of sources that have similar colors to either stars or galaxies, \gls{umap} is used \citep{mcinnes_umap_2018}. \gls{umap} perform a non-linear dimension reduction on data. It preseres local neighborhood information, which means that it retains the higher dimensional sutrcutre when reduin ght dimensionality. After the dimension reduction of the color data, DBSCAN \citep{ester_density-based_1996} is used to label the sources into two groups: one with sources that have star-like colors, and one with sources that have galaxy-like colors.
...
\begin{figure}[H]
    ...
    \caption{Comparison of \gls{snr} from \gls{x-bap} and the \gls{mer} catalog for all catalog filters. The dashed line denotes equal \gls{snr}. The measurements are from tiles 102022978 (top row), 102158274 (middle row and bottom row).}
...
Here, \gls{umap} \citep{mcinnes_umap_2018} is used to reduce the dimensionality of all the color information of the sources. This is done for the tiles in the \gls{edfn} and for the tiles in \gls{edff} and \gls{edfs}. The result of the splitting based on colors is shown in \autoref{fig: umap splitting}.
...
\end{figure}
\noindent Using \gls{umap}, 2 large clusters of objects are visible (\autoref{fig: umap splitting}). These are expected to be the sources that have galaxy-like colors and sources that have star-like colors. As \glspl{qso} can have much more varied colors, they are likely to be among both clusters. The structure of the clusters is similar between the 2 fields. The points with label 1 are more shaped as a blob, whereas the points with label 0 form a longer string. However, there is no information on which type of object corresponds to these clusters. For this, the colors of the clusters are plotted to show which type of object corresponds to which label.

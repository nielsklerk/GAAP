...
\end{equation}
where $O(x,y)$ is the observed data and $W_\mathrm{A}(x,y)$ some weight function.$O(x,y)$ is the convolution of the intrinsic image with the PSF
...
\end{equation}
with $\bar{P}$ the reflected PSF $\bar{P}(x,y)=P(-x,-y)$.
...
\begin{enumerate}
    ...
    \item Calculate the post-seeing aperture $W_\mathrm{A}^i$ by deconvolving the aperture function $\tilde{W}_\mathrm{A}$ by the reflected PSF $\bar{P}_i$, of band $i$; $W^i_\mathrm{A}=\tilde{W}_\mathrm{A}\otimes^{-1}\bar{P}_i$
    \item Calculate the aperture flux for each band using the post-seeing aperture function $W_\mathrm{A}^i$.
\end{enumerate}
The deconvolution can be calculated by examining \autoref{eq: weight convolution} in the frequency domain using the Fourier transform as a convolution in the \textbf{WHAT} domain becomes a multiplication in the frequency domain.
...
\end{equation}
However, \autoref{eq: analytical deconvolution} has a division by the Fourier transform of the reflected PSF, which can be close to 0 at some points. A division by a small number results in inflation of the machine error in the calculation, giving inaccurate results. 
...
If the noise varies across the image, the assumption made in \autoref{subsec: Noise Estimation from background Noise} does not hold anymore. In this case, the only assumption made to find the covariance matrix is that the correlation between two pixels does not depend on the location in the image. So the covariance matrix can be approximated like

\begin{equation}\label{eq: rms covariance}
    C(x,y;x',y') \approx \sigma(x,y)\rho(x-x',y-y')\sigma(x',y'),
\end{equation}
where $\sigma(x,y)$ is the standard deviation at position $(x,y)$ and $\rho(x-x',y-y')$ the correlation between the points $(x,y)$ and $(x',y')$. The values of $\sigma(x,y)$ are derived from the RMS map. To find the values for the correlation function, the covariance matrix $C(\Delta x, \Delta y)$ is used from \autoref{subsec: Noise Estimation from background Noise} by using 
...
\end{equation}
In the case that the noise in the pixels are uncorrelated, the only non-zero element of the correlation function is $\rho(0,0)=1$. This results in the expression simplifying to
...
To assess the accuracy of the error, different noise realizations are generated, and the flux $F_i$ and error $\sigma_i$ are computed for each realization. The standard deviation of the flux values is taken as the true standard deviation, $\sigma_\mathrm{True}=\mathrm{std}(F_i)$. The mean of the measured errors is used to compare with the true standard deviation. This is repeated for different combinations of weight and PSF sizes. 

The same process is repeated for Poisson noise. 

Next observations are created that include both Gaussian background noise and Poisson noise. The variances for both noise types are computed and summed to obtain the variance of the flux measurements. This is compared to the real variance of the flux values. 
...
To test the aperture shape that maximizes the SNR, the flux SNR is calculated for various scale parameters in both the x- and y-directions. 
...
To determine the weight that approximately maximizes the achievable SNR, a fit is performed for each source. We then use the deconvolution fit to determine the best original weight.  

\subsection{Applying GAAP Algorithm}


\section{Photometric Redshift Estimation}
Eazy \citep{2008ApJ...686.1503B}
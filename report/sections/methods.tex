...
In this chapter, the \gls{pca-phot} algorithm is explained and tested on simulated observations. Next, the technique is applied to Euclid Q1 observations. 

\section{PCA-PHOT}
\subsection{Flux Measurement}
Aperture photometry uses a weight function to find the flux of objects in observations. In general, it can be expressed in the following way
...
\end{equation}
The local covariance matrix $C(\Delta x, \Delta y)$ can be calculated using the formula
...
where $P$ is the \gls{psf}, $I$ the intrinsic image, and $N$ the Gaussian noise. The accuracy of the flux measurement is tested using Gaussian \glspl{psf} with Gaussian weights, varying their sizes, and computing both the true and measured fluxes.
...
To test the method for robustness against variations in the \gls{psf} used for the deconvolution compared to the true \gls{psf}, the flux of the object is calculated using \glspl{psf} of different sizes and compared to the true value of the flux.
...
For each galaxy, a cutout is made around it to determine the flux in that region. The cutout is 128 pixels for the \gls{mer} mosaic, which corresponds to a physical size of 12.8". 
% To get the same physical size for the Rubin images, the cutout is 64 pixels. 

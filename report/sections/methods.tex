...
\end{equation}
\begin{equation}\label{eq: weight convolution}
    \tilde{W}_\mathrm{A}(x,y) =(\bar{P}\otimes W_\mathrm{A})(x,y)
\end{equation}
...
\subsection{Noise Estimation from background Noise}

The most general form of the error on the flux is given by

\begin{equation}
    \sigma^2 = \sum_{x,y}\sum_{x',y'}W_\mathrm{A}(x,y)C(x,y;x',y')W_\mathrm{A}(x',y').
\end{equation}
Assuming that the covariance of pixels only depends on the distance between the pixels, the covariance becomes a local covariance matrix.
...
\begin{equation}
    \sigma^2=\sum_{x,y,x',y'}W(x,y)C(x,y,x',y')W(x',y')
\end{equation}
\begin{equation}
    \sigma^2=\sum_{x,y,x',y'}W(x,y)\sigma(x,y)\rho(x-x',y-y')\sigma(x',y')W(x',y')
\end{equation}
Define $W(x,y)\sigma(x,y)=W'(x,y)$
\begin{equation}
...
\end{equation}
For uncorrelated noise, this will be
\begin{equation}
...
\end{equation}
Gaussian noise is applied after the convolution to keep it uncorrelated and to use the uncorrelated-noise equation \autoref{eq: uncorrelated noise}. 

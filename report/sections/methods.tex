...
\begin{equation}
    \mathcal{F}\{\tilde{W}_A^i(x,y)\}=\mathcal{F}\{\bar{P}_i(x,y)\}\cdot\mathcal{F}\{W_A(x,y)\}
\end{equation}

which gives us 

\begin{equation}
    W^i_A = \mathcal{F}^{-1}\left\{\frac{\mathcal{F}\{\tilde{W}_A(x,y)\}}{\mathcal{F}\{\bar{P}_i(x,y)\}}\right\}
\end{equation}
...
Next, each component of the fiducial model is varied individually to investigate the consequence each different parts have on the measured flux and the estimated error.  

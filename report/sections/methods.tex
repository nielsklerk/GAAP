...
In this chapter, the \gls{pca-phot} algorithm is explained and tested on simulated observations. Next, the technique is applied to real observations in different broadband filters made by Rubin, Euclid, and \gls{decam}. 
...
To apply the \gls{pca-phot} method to images from both Euclid and Rubin, several inputs are required: \glspl{psf}, noise squares, and weight functions.
...
\subsection{Applying PCA-phot Algorithm}
For each galaxy, a cutout is made around it to determine the flux in that region. The cutout is 128 pixels for the \gls{mer} mosaic, which corresponds to 12.8". To get the same physical size for the Rubin images, the cutout is 64 pixels. 
...
\begin{equation}\label{eq: numerical deconvolution v2}
    W^i_\mathrm{A} = \mathcal{F}^{-1}\left\{\frac{\mathcal{F}\{\bar{P}_i\}^*}{|\mathcal{F}\{\bar{P}_i\}|^2+K}\cdot\mathcal{F}\{\tilde{W}_\mathrm{A}\}\cdot\exp(-2\pi i(k_xdx+k_ydx))\right\}.
\end{equation}
The size of the pre-seeing weight function is the \gls{fwhm} of the sources used in the \gls{mer} catalog. The flux is calculated for weight functions that are multiples of this \gls{fwhm}, following a similar procedure as \cite{SourceNeeded}. 

\section{Source Classification}
The colors calculated using the different sizes of the weight functions give information about the properties of the observed sources in the Euclid fields.
\subsection{Differentiating between Point-Sources and Extended Sources}
As \gls{pca-phot} is a way to apply a weight function on the pre-seeing image, point-sources are delta functions in this pre-seeing image. The weight functions are centered on the objects, which means that for point-sources, the flux does not change much as the value of the weight function at the center is constant for different sizes of the weight functions. However, for extended sources, increasing the size of the pre-seeing weight function increases the flux. This can be used to divide the sources into a group of point-sources and a group of extended sources.
\textbf{HOW TO DO THIS MORE PRECISELY}

\subsection{Point-Sources}
The point-sources in the images largely consist of stellar objects and \glspl{qso}. 

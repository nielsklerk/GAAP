...
In this chapter, the \gls{gaap} algorithm is explained and tested on simulated observations. Next, the technique is applied to real observations in different broadband filters made by Rubin, Euclid, and \gls{decam}. 

\section{GAAP}
\subsection{Flux Measurement}
Aperture photometry uses a weight function to find the flux of objects in an observation. In general, it can be written in the following way

\begin{equation}\label{eq: aperture photometry}
    F_\mathrm{A} = \int dxdyO(x,y)W_\mathrm{A}(x,y)
\end{equation}
where $O(x,y)$ is the observed image and $W_\mathrm{A}(x,y)$ some weight function. For example, in circular aperture photometry, this weight function is 1 for points within a given radius and 0 outside that radius. The observed image $O(x,y)$ is the convolution of an intrinsic image with the \gls{psf} with noise added to it.
...
\end{equation}
where it is assumed that the integral over the noise weighted by the weight function is 0 $\int dxdyN(x,y)W_\mathrm{A}(x,y)=0$.
...
To improve the numerical stability of \autoref{eq: analytical deconvolution}, a factor $K=10^{-8}$ is added to the equation to stabilize it 
...
\end{equation}
where $O(x,y)$ and $W_\mathrm{A}(x,y)$ are the pixel values of the observation and the post-seeing respectively. The uncertainty in the flux can be calculated using
...
\end{equation}
The fiducial weight function is a normalized Gaussian with a standard deviation of 8 pixels. The noise $N$ in the simulated image is drawn from a Gaussian distribution with mean 0 and variance 0.01. The resulting observation is then calculated using 
\begin{equation}
    O = P\otimes I + N.
\end{equation}
...
To test the method for robustness against variations in the \gls{psf} used for the deconvolution compared to the true \gls{psf}, the flux of the object is calculated using differently size \glspl{psf} and compared to the true value of the flux.
